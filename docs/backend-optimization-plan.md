...
### #7 FundNavScrapeService 用 HTTP 而非 HTTPS
...
### #9 DividendInfoService 与 FundDividendScrapeService 重复解析同一数据源
...
- **修复方案**：改为从基金详情接口获取 `establishDate`（基金成立日期），按每个基金的实际成立日期过滤。或至少将阈值改为可配置项 `${app.fund.min-valid-ex-date:2020-01-01}`。
...
### #12 weeklyChange/monthlyChange 口径错误
...
| 序号 | 问题 | 严重程度 | 文件 | 大白话 |
|------|------|----------|------|--------|
| #23 | EventService 无净值时用 1 元兜底 | 🟢低 | EventService.java:136-139, 222-225 | 分红复投时如果没有净值数据，系统用 1 元当净值，导致份额金额完全失真 |
