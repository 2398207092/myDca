...
**核心原则：不要就事论事，要识别"问题的类别"，然后防御整个类别。**

当用户提出一个 BUG 或问题时，AI 必须按以下步骤分析：

#### 第一步：分类
识别这个 BUG 属于哪个**问题类别**，例如：
- 数据隔离（多用户数据交叉）
- 并发安全（竞态条件、非幂等操作）
- 事务完整性（部分失败、回滚不完整）
- 级联操作完整性（部分删除、孤儿数据）
- 状态一致性（前端缓存 vs 后端数据不一致）
- 输入校验缺失（空值/非法值导致 500）
- 定时任务可靠性（幂等、重试、部分失败）
- 认证/授权缺失（接口未校验用户身份）
- 资源泄漏（连接未释放、线程未回收）
...
#### 第二步：全局扫描
在项目范围内扫描**同一类别**的所有潜在风险点，列出一个完整的清单。不要只修复用户指出的那一个点。

#### 第三步：防御方案
针对该问题类别，给出**系统性防御方案**，而非单点修复。方案必须包含：
1. **架构层**：如何让这类错误难以发生（框架约束、设计模式）
2. **自动化层**：如何让这类错误被机器发现（扫描规则、CI 检查）
3. **运行时层**：如何让这类错误被快速暴露（测试、监控）

#### 第四步：局限性标注
在方案末尾必须标注：
- 该方案覆盖了哪些问题类别
- **明确不覆盖哪些问题类别**（避免用户误以为一劳永逸）
- 剩余未覆盖类别中，哪些风险最高、建议优先处理

> **示例**：数据隔离问题 → 识别为"数据隔离类别" → 扫描所有 `@Modifying`、`@Scheduled`、POST/PUT/DELETE 接口 → 方案包括 Repository 基类强制 userId 参数、CI 扫描规则、多用户测试 → 标注"此方案不覆盖并发安全、事务完整性、前端状态一致性等问题，建议优先处理并发安全"
...
**此条为最高优先级规则，AI 不得以任何理由自行跳过。**

#### 触发条件
以下操作被定义为"生产端操作"，**必须**经用户明确授权后方可执行：
- `git push` / `git commit`（将变更写入远程仓库）
- `scp` 上传文件到服务器
- `ssh` 执行服务器命令（部署、重启、配置修改）
- `gh` / GitHub CLI 操作远程仓库
...
└──────────────────────────────────────────────────┘
    ↓
┌─ 步骤 3：收到授权词后 → 执行安全审查 → 执行操作 ─┐
│  (已有规则：审查敏感信息、评估安全影响)              │
└──────────────────────────────────────────────────┘
```

#### 关键约束
1. **禁止连续授权**：上一次授权仅对当次生效。下次需要推送/部署时，必须重新经历步骤 1-2
2. **禁止推断授权**：用户同意"修复代码" ≠ 同意"推送和部署"。修复和生产操作是两件事
3. **禁止自我授权**：AI 不得在用户提问格式中夹带"是否要推送"等诱导与代码修复在同一回合完成，必须修复完成后单独提出

在将代码推送至 GitHub 之前，AI 必须审查本次待推送的变更内容（源码、配置文件、注释、commit message）中是否包含硬编码的密码、Token、API Key、服务器 IP、邮箱授权码等敏感信息。若发现，则阻止推送并告知用户，待移除或替换为环境变量占位符后再推送。

在将代码部署至服务器之前，AI 必须评估部署操作对服务器安全的影响：不引入暴露敏感信息的配置、不修改已有的安全加固措施（防火墙规则、SSH 配置、端口绑定等）、不引入新的安全风险。若存在安全隐患，应提示用户并暂缓部署。
...
│   ├── profile/ProfilePage.vue     # 我的（含工具箱功能）（level 1）
...
### 部署的触发条件
只有当用户明确说出"推送 github"、"部署到服务器"、"上传服务器"、"更新服务器"、"部署"等包含明确推送/上传/部署意图的指令时，才执行以下任一操作：git push、scp 上传、ssh 部署脚本。用户仅表达"改好了"、"完成了"等模糊表述不视为部署指令。
