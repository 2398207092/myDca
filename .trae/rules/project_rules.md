...
当用户要求"推送/上传/部署/发布/更新到服务器"（或类似表达），AI 必须等待用户明确说出"推送/上传/部署"等确认指令后，才执行 git push、scp 上传 JAR、ssh 重启服务等生产端操作。不得在用户仅说"改好了"、"完成了"、"可以了"等模糊表达时主动推送到 GitHub 或服务器。目的是避免未经确认的变更进入生产环境。
